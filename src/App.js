...
function Welcome(props) {
	return (
		<div>
			...
			<p>You need to eat all the mushrooms in 64 steps. By Default maze size is 10 x 10, you can change it by reloading or entering the height and width in prompt.</p>
		</div>
	)
}
...
function ScoreCard(props) {
	...
	let mushrooms_remaining = document.getElementById('mushrooms_remaining')
	steps_remaining.innerHTML = max_no_of_moves - no_of_moves
	no_of_moves_score.innerHTML = no_of_moves
	mushrooms_remaining.innerHTML = document.getElementsByClassName('active').length
...
class App extends Component {
	...
	handleLoad(width, height) {
		...
		for (let i = 0; i < truncated_data.length; i++) {
			...
			elem_position.innerHTML = "<img src='mario-mushroom.jpeg' alt='mario' class='maze-image'/>";
		...
		marioposition.innerHTML = "<img src='mario-icon.jpg' alt='mario' class='maze-image'/>";
